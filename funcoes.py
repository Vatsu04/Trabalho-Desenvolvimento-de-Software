...
def calcular_desconto(preco_inicial, taxa_desconto, response):
    ...
    text_response = "O preço com desconto é: R$" + str(round(preco_novo, 2))
    response.config(text = text_response)
    
    registrar_relatorio('calcular_desconto', str(float(preco_inicial)) + ',' + str(float(taxa_desconto)), str(round(preco_novo, 2)))
...
def calcular_acrescimo(preco_inicial, taxa_acrescimo, response):
    ...
    text_response = "O preço com acréscimo é: R$" + str(round(preco_novo, 2))
    response.config(text = text_response)
    registrar_relatorio('calcular_acrescimo', str(float(preco_inicial)) + ',' + str(float(taxa_acrescimo)), str(round(preco_novo, 2)))
    
    
def calcular_montante(capital, taxa_juros, tempo, response):
    montante = float(capital) * (1 + (float(taxa_juros) / 100)) ** float(tempo)
    
    text_response = "O montante é: R$" + str(round(montante, 2))
    response.config(text = text_response)
    registrar_relatorio('calcular_montante', str(float(capital)) + ',' + str(float(taxa_juros)) + ',' + str(float(tempo)), str(round(montante, 2)))
...
def calcular_juros_simples(valor_principal, taxa_juros, tempo, response):
    ...
    text_response = "O montante com juros simples é: R$" + str(round(montante, 2))
    response.config(text = text_response)
    registrar_relatorio('calcular_juros_simples', str(float(valor_principal)) + ',' + str(float(taxa_juros)) + ',' + str(float(tempo)), str(round(montante, 2)))
...
def calcular_juros_compostos(valor_principal, taxa_juros, tempo, response):
    ...
    text_response = "O montante com juros composetos é: R$" + str(round(montante, 2))
    response.config(text = text_response)
    registrar_relatorio('calcular_juros_compostos', str(float(valor_principal)) + ',' + str(float(taxa_juros)) + ',' + str(float(tempo)), str(round(montante, 2)))
...
def lucro(custo, receita, response):
    ...
    text_response = "Lucro: R$" + str(lucro) + "\nPercentual de Lucro: " + str(percentual) + "%"
    response.config(text = text_response)
    registrar_relatorio('lucro', str(float(custo)) + ',' + str(float(receita)), str(round(percentual)))
...
def prejuizo(custo, receita, response):
    ...
    text_response = "Prejuízo: R$" + str(prejuizo) + "\nPercentual de Prejuízo: " + str(percentual) + "%"
    response.config(text = text_response)
    registrar_relatorio('prejuizo', str(float(custo)) + ',' + str(float(receita)), str(round(percentual)))
...
def porcentagem(valor, porcentagem, response_text):
    ...
    registrar_relatorio('porcentagem', str(float(valor)) + ',' + str(float(porcentagem)), str(resultado))
    

def calcular_roi(ganho_investimento, custo_investimento, label):
    roi = (float(ganho_investimento) - float(custo_investimento)) / float(custo_investimento) * 100
    
    label.config(text="O retorno sobre o investimento \n (ROI) é: " + str(roi) + "%")
    registrar_relatorio('calcular_roi', str(float(ganho_investimento)) + ',' + str(float(custo_investimento)), str(roi))
...
def calcular_tir(vpl, pv1, pv2, i1, i2, response):
    try:
        ...
        registrar_relatorio('calcular_tir', str(float(vpl)) + ',' + str(float(pv1)) + ',' + str(float(pv2)) + ',' + str(float(i1)) + ',' + str(float(i2)), str(tir))
...
def calcular_vpl_handler(taxa, fluxo_caixa_str, investimento_inicial, response_text):
    ...
    registrar_relatorio('calcular_vpl', str(float(taxa)) + ',' + str(float(fluxo_caixa_str)) + ',' + str(float(investimento_inicial)), str(vpl))
    
    
    

def registrar_relatorio(ferramenta_utilizada, entrada, saida):
    connection = connect()
    cursor = connection.cursor()
    ...
    VALUES (%s, %s, %s, %s, %s)
    """
    valores = (user_id, data, ferramenta_utilizada, entrada, saida)
